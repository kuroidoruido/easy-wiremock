    ...
    const lastSeenVersion = useQuery({
        ...
        queryFn: () =>
            new Promise<LastSeenVersion>((resolve) => {
                const stateInLocalStorage = localStorage.getItem(
                    LOCAL_STORAGE_LAST_SEEN_KEY
                );
...
function tagSeen(changelog: Changelog | undefined, lastSeenVersion: LastSeenVersion | undefined): Changelog | undefined {
    let areSeen = false;
    const versions: ChangelogVersion[] = [];
