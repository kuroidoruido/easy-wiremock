...
export function ServerCard({ server, onEdit, onRemove }: ServerCardProps) {
  ...
  return (
    <article className="server-card" onClick={() => Router.push("Wiremock", { serverId: server.id })}>
      <main>
        <h3>{server.label}</h3>
        <p>{server.url}</p>
      ...
      <footer>
        <button type="button" className="btn-dropdown secondary" onClick={onActionClick}>
          ...
          <ul className="btn-dropdown-menu">
            <li role="button" onClick={() => onEdit(server.id)}>
              Edit
            </li>
            <li role="button" onClick={() => onRemove(server.id)}>
              Remove
            </li>
          </ul>
        </button>
